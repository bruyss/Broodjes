...
subjects = ('Broodje', 'broodje', 'Broodjes', 'broodjes', 'Bestelling broodje',
            'Bestelling broodjes', 'bestelling broodje', 'bestelling broodjes')
...
def getmailIDs(subject):
    """Geeft mail IDs van ongelezen emails die vandaag ontvangen zijn met
    bepaald onderwerp"""
...
def getmail(imapObj, ID):
    """Geeft verstuurder en eerste lijn van body van een email"""
...
def initworksheet():
    """Initialisering van een nieuwe excel voor bestellingen"""
...
def getnieuwebestellingen():
    """Haalt alle nieuwe bestellingen van de mailserver"""
    nieuwebestellers = []
    nieuwebestellingen = []
    ...
    for ID in IDs:
        ...
        logging.warning('Bestelling ontvangen van %s: %s voor %s' %
                        (besteller[1], bestelling, besteller[0]))
        print('%s voor %s vanuit %s' %
              (bestelling, besteller[0], besteller[1]))
        sendmail('Robobroodje', besteller[1],
                 'Bestelling broodje',
                 """
                 Beste %s\n\nU bestelling van %s is goed ontvangen en zal deze
                  middag klaarliggen.\n\nMvg\nRobobroodje
                  """.format(besteller[0], bestelling),
                 [],
                 'smtp.gmail.com',
                 587,
                 address,
...
def vulexcel(wsnaam, bestellers, bestellingen):
    """Vult excel aan met bestellingen"""
...
def sleeper():
    ...
    morgenvroeg = datetime.datetime(
        morgen.year, morgen.month, morgen.day + 1, 5)
    while datetime.datetime.now() < morgenvroeg and morgenvroeg.weekday() < 6:
        time.sleep(10)
